...
public class ASTEntityBuilder {//don't serialize AST information
	...
	protected ASTEntity generateEntityForNode(ASTEntity parent, ASTNode node) {
		...
		if(node instanceof AbstractTypeDeclaration) {
			...
			String implementation = targetObject.getContent().substring(startPosition, endPosition).replace("@SuppressWarnings(\"unchecked\")", "");
			implementation = CodeManipulation.removeComments(implementation);
			//TODO: use a more general solution to remove compiler instructions
			...
			entity = new ASTEntity(returnType, name, "", bodyImplementation, startPosition);
			{
				...
				String argumentDeclaration = bodyStart==-1?unparemetrizedImplementation:unparemetrizedImplementation.substring(typeDeclarationEnd,bodyStart);
			...
			while(specialJavadoc!=-1) {
				...
				if(tmpText.startsWith("@author") || tmpText.startsWith("@throws")) {
					...
					if(firstSpace!=-1) {
						...
						String comment = CleanComments.clean(paramComment.substring(firstSpace).trim());
						if(!comment.isEmpty()) {
			...
			commentText = CleanComments.clean(commentText);
